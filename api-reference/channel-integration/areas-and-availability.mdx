...
- **`busy[].subject`** - what holds the court for that interval: `member_booking`, `class`, `event`, `area_block` (the gym closed the court for maintenance or a private hire), `time_off`, or `shift_break`. Same vocabulary as `details.conflicts[].subject` on a [refused booking](/api-reference/channel-integration/bookings#conflicts), so an hour you cannot sell can be explained rather than shown as unattributed busy time. No id or name is attached - an availability read is not a read of who booked. Use [List bookings](/api-reference/channel-integration/bookings#list-bookings) for the rows you may see.
