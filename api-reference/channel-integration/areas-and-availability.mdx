...
A **closure** takes an area off the bookable schedule for a period: resurfacing, a holiday, a camp. Availability already reflects closures, so a closed slot simply won't appear in `slots`. Read closures directly when you want the *reason* and the period to show in your own calendar, or when migrating a schedule into 1Club.

A closure with `areaId: null` is gym-wide: it closes every area in its `clubId`. If you key your calendar by court, expand it against that gym's areas.
...
```bash
curl "https://api.1club.ai/v1/platform/area-closures?from=2026-08-01T00:00:00Z&to=2026-08-08T00:00:00Z" \
  -H "Authorization: Bearer 1club_sk_live_..."
```
...
- `clubId` (optional) - only closures for this club
- `areaId` (optional) - only closures affecting this area, gym-wide ones included

Any closure that *overlaps* the window is returned, so a week-long closure still appears when you ask about one day inside it.
...
```bash
curl -X POST "https://api.1club.ai/v1/platform/area-closures" \
  -H "Authorization: Bearer 1club_sk_live_..." \
...
- `areaId` (optional) - the court to close. Omit or send `null` for a gym-wide closure.
...
Returns `201` with the created closure.

<Note>
  A closure blocks **new** bookings. Bookings already inside the period are not
  cancelled, so migrate or cancel those separately if you need the period clear.
</Note>

### Delete a closure

Requires the `areas:write` scope. Deleting a closure reopens the period for booking.

```bash
curl -X DELETE "https://api.1club.ai/v1/platform/area-closures/12" \
  -H "Authorization: Bearer 1club_sk_live_..."
```
...
<Note>
  Creating or deleting a closure emits an availability change for every affected
  area, so the
  ...
  re-pull those courts. A gym-wide closure emits one change per area in the gym.
</Note>
