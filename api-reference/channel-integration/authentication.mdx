...
Booking creation accepts an optional **`Idempotency-Key`** header. Send a unique key per booking attempt (for example your own booking id); if the request is retried with the same key, 1Club returns the original booking instead of creating a duplicate. Reusing a key for a genuinely different booking returns `409 Conflict`.
