...
```mermaid
flowchart TD
  A["Partner<br/>POST /v1/platform/bookings<br/>{ customer.email, areaId, time, payment }"] --> B{Area bookable<br/>through this API?}
  B -- no --> E1["400 · not bookable"]
  B -- yes --> C["Normalize email<br/>(trim + lower-case)"]
  C --> D{"Contact with this email<br/>in this organization?"}
  D -- "yes" --> F["Reuse existing contact<br/>(name/phone not overwritten)"]
  D -- "no" --> G["Create contact<br/>email, first/last name, phone"]
  F --> H["Create booking → link contactId<br/>atomic availability re-check"]
  G --> H
  H -- "slot taken" --> E2["409 · not available"]
  H -- "ok" --> I["Record transaction<br/>paid → settled · unpaid → outstanding"]
  I --> J["201 · { bookingId, status, payment }"]
```

Contact resolution runs **before** the booking write. So if the slot turns out to be
taken (`409`) or the request is otherwise rejected, a contact that was just created
for a first-time customer is **kept** — it isn't rolled back. That's harmless: it
carries no booking, stays matched by email, and is reused when you retry. The booking
write itself is what re-checks availability atomically and rejects conflicts.
