...
Requires the `bookings:write` scope. Send an optional `Idempotency-Key` header so retries don't create duplicates.

```bash
curl -X POST "https://api.1club.ai/v1/platform/bookings" \
  -H "Authorization: Bearer 1club_sk_live_..." \
  -H "Idempotency-Key: padel-app-bk-9931" \
...
- `payment.amount` (optional) - amount collected; defaults to the area's price for the slot
...
If you send an `Idempotency-Key` and retry the request with the same key, 1Club returns the original booking with HTTP `200` and `idempotentReplay: true` instead of creating a duplicate. Reusing a key for a different area or time returns `409 Conflict`.

### Common errors

- `409` - the area is not available for the selected time, or the Idempotency-Key was reused for a different booking.
