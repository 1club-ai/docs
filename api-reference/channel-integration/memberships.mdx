...
- `usesRemaining` - sessions left right now, which is how you migrate a package the customer has already partly used
...
it. This is the same set of fields the gym's
staff can change in the **Add membership** dialog, so an API sale and a front-desk
sale can express the same deal.
