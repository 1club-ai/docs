...
1Club sends a signed HTTP `POST` to your endpoint whenever a court's
...
In the admin dashboard under **Settings > Organization > APIs & webhooks**, add a webhook endpoint:
...
Changes are dispatched by a worker that runs **every 30 seconds**, so a first
delivery normally arrives within about half a minute of the change, plus network
time. Treat the integration as near-real-time rather than instant: design the
booking flow so a court can be sold on either side during that window, and let the
[atomic re-check on create](/api-reference/channel-integration/bookings#conflicts)
be what stops a double-booking.

<Warning>
  Do not measure delivery latency against a **staging** or trial environment. Those
  run with CPU allocated only while serving requests, which starves the dispatch
  worker between ticks and inflates the figure well past what production does.
</Warning>
...
- **Auto-disable.** After the 7th attempt fails, the endpoint is **disabled automatically** and the reason is shown under **Settings > Organization > APIs & webhooks**. Re-enable it (and rotate the secret if needed) once healthy, then reconcile with the change feed to catch up on everything sent while it was down.
...
- **Deliveries stopped arriving.** The endpoint was likely auto-disabled after repeated failures - check **Settings > Organization > APIs & webhooks** for the disable reason, fix your endpoint, re-enable it, then reconcile from your last `cursor`.
