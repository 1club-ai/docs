...
The same Platform API is exposed over the Model Context Protocol at `https://mcp.1club.ai/admin`, so Claude, ChatGPT, and other MCP clients can call it directly using the API token you created above. The MCP tools are thin wrappers over these endpoints, with the same scopes, rate limits, and error shapes.
