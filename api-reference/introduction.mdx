...
description: "Integrate with the 1Club platform to access clubs, classes, bookings, plans, instructors, and more."
---

The 1Club Platform API lets you programmatically access and manage your organization's data — clubs, classes, bookings, content, and more — from your own websites, integrations, and back-office scripts.
...
Grant only the scopes the integration actually needs. For example, the Bookings endpoints require the `bookings:read` scope.
...
```bash
curl "https://api.1club.ai/v1/platform/bookings?startDate=2026-05-01&endDate=2026-05-31" \
  -H "Authorization: Bearer 1club_sk_live_..."
```
