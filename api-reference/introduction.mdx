...
The same Platform API is exposed over the Model Context Protocol at `https://mcp.1club.ai/admin`. Remote MCP clients connect with 1Club OAuth, not with the API key created above. The MCP tools are thin wrappers over these endpoints, with the same scopes, rate limits, and error shapes. See [Connect an assistant](/mcp/connect).

See the [MCP overview](/mcp/overview) for the two servers 1Club runs, and [Connect an assistant](/mcp/connect) for setup.
