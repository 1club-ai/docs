...
A successful scheduled run sends a notification to admins subscribed to billing notifications. There's no email to members from the bill run itself - that's a separate action on each invoice.
