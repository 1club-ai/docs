...
Each document type has its **own** template and its **own** counter, set per company under **Settings > Billing > General**. Most gyms have one company, so there is one pair of templates to fill in; if you trade through several, each keeps its own series and its own templates. See [Billing entities](/billing/billing-entities).
...
Each counter belongs to the company issuing the document and is assigned under a lock scoped to that series, so numbers stay unique and gapless for that company even when two staff members create documents at the same moment. Two companies number independently, so both can issue a document numbered 1 - give each a distinct prefix.
...
| Two invoices almost got the same number                                          | Concurrent creation.                                                          | No action needed - numbering retries on collision and stays unique for the issuing company.  |
