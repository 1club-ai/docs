...
- For memberships flagged **payment required up front**, it attempts to charge the contact's default Stripe payment method when the recurrence becomes due. This requires Stripe to be connected; if it is not, no charge is attempted and the transaction stays `pending` for manual payment. If no default method is on file or the charge fails, the balance stays outstanding and a payment-failed notice is emitted so staff (and the member) can follow up.
- Marks any `pending` transaction whose due date is in the past as `overdue`.

It does not send invoices and it does not automatically retry failed charges. Retries are manual: open the transaction or invoice and use **Charge** or **Add payment**.

For the member's view of all this - checkout, saved cards, wallet credit, and refunds - see [How members pay](/billing/member-payments).
...
- [How members pay](/billing/member-payments) - the member-facing view of checkout, wallet, and refunds.
