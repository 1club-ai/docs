...
- **Daily check-in limit**. Optional cap on membership entries per member per day, counted in the gym's timezone. Leave empty for unlimited. The cap applies to membership check-ins at the door (kiosk, QR scanner, member portal); it does not limit check-ins for bookings or classes, and front-desk staff can always check a member in manually. Editing the limit takes effect immediately for existing memberships on the plan.
