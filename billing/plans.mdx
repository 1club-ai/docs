...
- **Daily check-in limit**. Optional cap on how many attendances the plan covers per member per day - class or area bookings funded by the membership plus door entries, combined, counted in the gym's timezone. Leave empty for unlimited. The cap is applied when the member books (an over-limit booking is simply not covered by the membership) and at self-service check-in (kiosk, QR scanner, member portal). Front-desk staff see a warning when a member is over the limit but can still check them in. Editing the limit takes effect immediately for existing memberships on the plan.
...
<Note>
  Booking policies no longer keep their own upsell order. When a policy upsells
  plans, they display in the global plan order set here. See [Booking
  Policies](/operations/booking-policies).
</Note>
