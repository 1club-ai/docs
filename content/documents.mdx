---
title: Documents
description: Create, edit, and manage documents for your website
---

**In a nutshell:** Create and manage documents with a powerful editor that includes rich text formatting, media management, and SEO optimization tools.
...
The Content list (under **CMS → Content**) groups pages by **content type** and renders each group as a sortable list. To reorder pages within a type, **drag the row** by its handle to the new position - the order saves automatically. The same shared drag-drop component is used on [Areas](/operations/areas), so the interaction is identical.

There is **no longer a manual sort-order field** in the page editor. Drag-and-drop is the only writer. If two admins drag in the same group at the same time, the second save returns a `409 REORDER_STALE` conflict and the page reloads with the latest order; redo the drag.
