...
3. In **Valid for**, switch to **Selected bookings**, turn on **Areas** and pick your court type. Under **Gyms**, pick the gym running the offer. An auto-apply discount needs at least one booking type, so **All bookings** can't be saved with **Auto-apply** on.
4. Under **Conditions**, turn on **Only at certain times** and set 08:00 to 16:00 with **Starts in the window**, and set **Booking length** to exactly 2 hours (120 minutes minimum and maximum). Leave the days empty for every day.
