...
- **Gyms** - applies only to bookings at the selected gyms. Leave empty for every gym. Use it when two of your gyms share the same area type but the offer is for one of them.
...
- **Time of day** - weekdays (optional; empty means every day) plus a start and end time, read on the gym's clock. Choose how the booking must match the window:
...
- **Booking length** - a minimum and maximum length. Set both to the same value to target one length exactly, for example 2 hours for a "2 for 1" offer.

Time of day and booking length judge the whole booking: the discount applies to a booking or it does not. They are never split at the window's edges the way a [pricing rule](/settings/pricing-rules) band is.
...
- When a member picks a shorter booking and a longer one would qualify, the calendar tells them - "Buy 1 get 1: add 1 h for free" - with a button that switches to the longer booking.

Signed-in members see offers they qualify for. Visitors who are not signed in only see offers with no audience, first-booking or per-member conditions, because the calendar cannot check those for them. Checkout always has the final say.

### Example: 2 hours for the price of 1 on daytime courts

A padel gym wants a second hour free on weekday daytime bookings at one of its gyms:
...
3. In **Valid for**, pick **Area types** (your court type) and, under **Gyms**, the gym running the offer.
4. Under **Conditions**, set **Time of day** to Monday to Friday, 08:00 to 16:00, **Starts in the window**, and **Booking length** to exactly 2 hours.
