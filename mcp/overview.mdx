...
- The **admin server** reads and writes one organization's own data. You sign in to 1Club, choose the organization, and approve the requested scopes. Every response is limited to that organization. This is the one you want for running your gym.
...
- **Scopes are the limit.** An assistant can only use tools approved during authorization. A connection with `classes:read` cannot create or update a class.
- **Revocation is instant.** Revoking a connection under **Settings > Artificial Intelligence > Connect AI assistants (MCP)** cuts the assistant off immediately.
- **Assistants can be talked into things.** Content the assistant reads, such as a member note or a website page, can contain text that tries to steer it. This is why read-only scopes are safer, and why write scopes belong only on connections you supervise.
...
- Your MCP client must support remote HTTP MCP servers and OAuth. [Connect an assistant](/mcp/connect) covers the setup.
