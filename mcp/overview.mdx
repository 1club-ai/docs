...
- The **discovery server** searches the public marketplace across all listed gyms and finds bookable court times. It is anonymous by design, read-only, and can never reach a single organization's private data.
...
The discovery server answers three questions about the public marketplace: when can I play (`find_availability`, which returns priced, dated court times with a link that opens the booking), which clubs match what I am after (`search_clubs`), and what is this place like (`get_club`). It reads only published information and never writes - a booking is completed on 1club.me.
