...
- **Write access is a deliberate opt-in, per capability.** Tick only the writes the assistant actually needs. Content it reads - a member note, a website page - can contain text that tries to steer it, so a write scope belongs only on a connection you supervise.
