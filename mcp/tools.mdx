...
- **A client that requests nothing gets a read-only connection.** Every read scope in the table above, and nothing that writes.
...
- **Requested write scopes are opt-in, one at a time.** Each appears under "Also allow it to make changes?" as a checkbox that starts unchecked. Approving the page without ticking any of them produces a read-only connection, even if the client asked for everything.
...
<Note>
  The wildcard scope `*` is an API-key feature: on a Platform API key it satisfies
  every current and future scope. MCP OAuth rejects it - a client that asks for
  `*` gets an `invalid_scope` error and has to name the scopes it wants. Even for
  a key, [explicit scopes are the better
