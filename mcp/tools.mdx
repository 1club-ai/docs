...
<Note>
  The wildcard scope `*` means "everything", and the two credential types treat it
  differently. On a Platform API key it is stored as-is and satisfies every current
  and future scope. MCP OAuth accepts a request for `*` but never stores it: the
  wildcard is expanded into the concrete scopes that exist at that moment, so the
  authorization page can list each one and you can leave the destructive ones
  unticked. A connection therefore does not silently gain new powers as 1Club adds
  scopes - reconnect when you want the newer ones. For a key, [explicit scopes are
  the better choice](/settings/api-tokens#token-scopes) unless the integration is
  fully trusted and internally managed.
</Note>
