...
<Note>
Before you start: how strict your check-in flow is depends on your booking policy and access-control settings. Read [Check-in Models](/reference/check-in-models) for an overview, and configure the rules in [Booking Policies](/operations/booking-policies) and [Access Control](/settings/access-control).
</Note>
...
<Note>
  **Pre-booked sessions honor the booking, even on an exhausted membership.** When the booking was created against a membership, that session has already been debited from the membership at booking time. Check-in succeeds even if the membership has since become inactive or hit zero remaining uses - the booking is the receipt. This only applies to check-ins that select the booking; walk-ins and entrance-method changes still require an active membership with remaining uses.
</Note>
