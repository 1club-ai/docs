...
A [season pass](/operations/season-passes) appears here too, sold from the schedule rather than from this page. It behaves differently in three places, each noted below: its allowance, its cancel dialog, and its renewal.
...
- **Season passes** open their own dialog instead. Ending one has to release the courts and close the pass together, so the generic cancel is refused for them. See [Cancelling a season pass](/operations/season-passes#cancelling-a-season-pass).
...
<Note>
  A [season pass](/operations/season-passes) reads its allowance the other way round. The sale spends the season's hours up front on the weeks it books, so a freshly sold pass shows its allowance fully committed here (`27 / 27 hours`), and `0 of 27 hours left` on member-facing surfaces. It rests there for the whole season, is never marked **Used** at zero, and the counter rises only when a cancelled week credits a makeup hour back. Its two real ends are the season's last day and cancelling the pass.
</Note>
...
A [season pass](/operations/season-passes) never auto-renews. It is paid once for a fixed period, and the next season is a fresh sale at the new season's prices.
