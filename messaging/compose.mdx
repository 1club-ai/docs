...
If you have access to custom templates (admin and manager roles), the **Template** picker above the body lists active templates of type `message` from `/v1/messages/templates`. Selecting one populates the subject and body in place. Edit either field after applying the template; the original template is not modified.
