...
- **Staff can override it.** The refusal offers **Book anyway** to anyone with
  manage-level booking permission, and the override is recorded on the individual
  occurrences that actually fell outside the window - not on the whole series.
...
Display order follows the global plan order set on the Plans page - see [Membership plans](/billing/plans).
...
If the email already belongs to an active member, the visitor is routed to log in instead. This is off by default - turn it on per policy when you want to let new visitors book before signing up. See [Member onboarding](/workflows/member-onboarding) for the full signup and verification experience.
...
the series (an earlier end date or fewer repeats), raise **Max. ahead**, set it to
repeat with no end, or use **Book anyway** if you hold manage-level booking permission.
