...
The detail page header shows the booking title with status chips (confirmed, pending, cancelled, paid/unpaid) and a row of actions: **Add payment**, **Notify participants**, **Edit**, **Cancel**, and **Delete**.

The body uses a two-column layout. The left column is the info table: created at, created by (with source - admin, member portal, public website, kiosk, or system), customer or coach, recurrence summary, entrance method, and any discount. The right column lists players (for area bookings with multiple participants) and shows a yellow note card if the booking has notes.
