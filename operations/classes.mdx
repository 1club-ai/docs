...
- **One-off or recurring** - repeat daily, weekly (by weekday), or monthly, with per-instance or all-future edits. A series with an end date is created in full; see [Repeating schedules](/reference/repeating-schedules).
...
- **Visibility** - **Public**, **Member only**, or **Private**.
- **Recurrence** - turn on to repeat. Pick the pattern (daily, weekly with specific weekdays, monthly) and how the series ends: **Until date**, **After N times**, or **Never**.

A recurring class is stored as a series of N separate class occurrences, each with its own bookings, attendance, and check-ins. Editing or deleting offers a scope choice: **This instance** or **All future**.

A series that ends on a date or after a set number of repeats is created in full when you save, so the whole term shows on the schedule immediately. **Never** keeps a rolling eight-week horizon and extends itself each night, because there is no end to create up to. See [Repeating schedules](/reference/repeating-schedules) for the full model and the 520-occurrence limit.
...
Turn on **Recurrence**, pick weekly with the right weekdays, and set **Ends** to **Until date** with the last day of the term. Assign the instructor and area; every week from now to that date becomes an occurrence, created the moment you save, so the court reads as occupied for the whole term. Editing offers **This instance** or **All future**, so you can move one week's class without disturbing the series.
...
**Issue**: Only the next couple of months of a recurring class appear on the schedule.

**Solution**: The series ends in **Never**, which keeps a rolling eight-week horizon. Edit it with the **All future** scope and set **Ends** to **Until date** with the real last day. See [Repeating schedules](/reference/repeating-schedules).
