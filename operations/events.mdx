...
- **One-off happenings** with their own public page - workshops, seminars, tournaments, open days, parties.
- **Ticket tiers** - multiple ticket types, each with its own price and capacity.
- **AI drafting** - ask the assistant to plan an event, grounded in your real gyms, sports, instructors, and areas.
- **Flexible payment** - pay online, pay on arrival, or let members choose.
- **Promote in one click** - open a prefilled message to your members; feature events across app, portal, and website.
- **Sell anywhere** - online, or at the front desk via POS, with an attendee list and check-in (and collect) on the day.
- **Group orders** - a buyer can purchase several seats, including mixed tiers, with guest registrations.
...
### Padel tournament with tiers

Create the tournament event, add two tickets - **Player** (capped at your draw size) and **Spectator** (unlimited, lower price) - and set payment to **Pay online** so entries are confirmed and paid up front. Promote it to your padel players and watch the per-ticket "N sold" counts on the detail page.

### AI-drafted yoga workshop

From the suggestion card (or by asking the assistant), draft a "Saturday morning yoga workshop". It arrives as a private draft with a free ticket, a future date, and a cover image from your media library. Review the description and time, add a paid ticket if you want, and publish.

### Martial-arts grading day, pay on arrival

Set the grading event's ticket to **Pay on arrival** so students reserve now and pay at the door. On the day, use **Check in & collect** on the attendee list to take payment and mark them present in one step.

### Club open day you want everyone to see

Make the event **Public** and turn on **Featured** so it surfaces on the app home rail, the portal, and the website. Use **Promote** to email members with a prefilled message, then unfeature it once the day has passed.

## Tips & best practices

- **Reach for an event, not a class, when you need tiers, a landing page, or promotion.** Use a class for recurring, single-price group activities with a waitlist.
- **Free tickets skip payment entirely** - ideal for community sessions and RSVPs.
- **Feature your next headline event, then unfeature it.** Featured rails self-hide when nothing qualifies, so keeping this tidy keeps member surfaces clean.
- **Assign guest seats at the door.** Turning an anonymous guest into a named contact puts the visit on their profile and helps future marketing.
- **Remember area assignment is visual only** - an event shown "in Court 1" does not hold Court 1; block it with a booking if you need the space reserved.

## Troubleshooting

**Issue**: The **Unpublish** action is missing.

**Solution**: You can only unpublish a live event while it has no active bookings. Once seats are booked, use **Cancel event** instead.

**Issue**: I can't delete a ticket.

**Solution**: A ticket with bookings can't be deleted. Set it to inactive to stop new sales while preserving existing orders.

**Issue**: Members can't see my event.

**Solution**: Check it's **Live**, not a draft, and that visibility isn't **Private** (or **Member only** for logged-out visitors).

**Issue**: A member's plan didn't cover the ticket.

**Solution**: By design - event tickets are always paid directly; memberships and external programs don't cover them.
