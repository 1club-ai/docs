...
Every club has its own front-desk culture. Some require members to book and pay before they ever set foot in the building. Others welcome walk-ins as long as the member has a paid membership or pays at the door. A third group — community gyms, martial-arts schools, kids' programs — log attendance on the spot and trust staff to handle payment separately.

You don't pick a model in one place. Instead, two settings work together:

- **Booking Policies** (`Settings → Booking Policies`) — control whether payment is required at booking or at check-in, and whether bookings need a contact at all. See [Booking Policies](/operations/booking-policies).
- **Access Control Settings** (`Settings → Access Control Settings`) — control whether walk-ins without a member account can be checked in. See [Access Control](/settings/access-control).
...
- In your booking policy, leave **Require Payment Upfront** off.
- Turn **Require Payment at Check-in** on.
...
- In your booking policy, leave both **Require Payment Upfront** and **Require Payment at Check-in** off.
...
Which entrance methods are required at check-in depends on your booking policy. If **Require Payment at Check-in** is on, the staff must select one of the three before the system records the check-in. If it's off, the front desk can record the check-in without selecting a method.

## Picking your model

Most clubs settle on one model per club, sometimes with a different policy for personal training or special programs. If you run multiple clubs under one organization, you can scope different booking policies to each — see the **Scope** section in [Booking Policies](/operations/booking-policies).
