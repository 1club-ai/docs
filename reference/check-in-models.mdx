...
- **Membership** - A check-in counts against the member's active membership. Usage is decremented automatically if the plan has a limit. By default members can enter as often as they like; set a **Daily check-in limit** on the plan to cap self-service entries per day (see [Plans](/billing/plans)).
