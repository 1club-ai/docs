...
- **Membership** - A check-in counts against the member's active membership. Usage is decremented automatically if the plan has a limit. By default members can enter as often as they like; set a **Daily check-in limit** on the plan to cap the attendances it covers per member per day - bookings plus door entries (see [Plans](/billing/plans)). The limit binds at booking and at self-service check-in; front-desk staff are warned but can proceed.
