---
title: API tokens
description: Create, scope, edit, and revoke API tokens for programmatic integrations, from Settings > Organization > APIs & webhooks.
keywords: [api tokens, api keys, scopes, programmatic access, integrations, webhooks, revoke token]
---
...
- **Webhooks** remain on this page. Register endpoint URLs that 1Club calls when availability changes, each with its own signing secret. See the [Webhooks reference](/api-reference/channel-integration/webhooks).
- **MCP** is separate from API tokens. Open **Settings > Artificial Intelligence > Connect AI assistants (MCP)** to copy the endpoint, review connected assistants, and revoke OAuth grants. See [Connect an assistant](/mcp/connect).
