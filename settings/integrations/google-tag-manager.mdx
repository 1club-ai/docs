...
description: Load a Google Tag Manager container on your member portal and CMS website with no code, then track page views, sign-ups, bookings, and purchases through your own tags.
keywords: ["google tag manager", "gtm", "ga4", "google analytics", "conversion tracking", "datalayer", "member portal analytics", "website tracking", "meta pixel"]
---
...
Google Tag Manager is a paid-plan integration. Organizations on the Free plan or still in the Grow trial cannot save a container ID until they upgrade.
...
- Test with [Tag Assistant](https://tagassistant.google.com) on both the portal and the CMS website after saving, since they load the same container independently.
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Tag not firing at all | The ID format is wrong or the form wasn't saved | Confirm the ID starts with `GTM-` (uppercase, letters and digits only) and that you saved, then check with [Tag Assistant](https://tagassistant.google.com) |
| Container loads but no events in GA4 | The GA4 tag or its triggers aren't set up, or the workspace wasn't published | Confirm the GA4 tag fires on the right triggers in GTM and that you published the latest version |
| Tag loads on portal but not website (or vice versa) | The page is serving a cached HTML response or the public org payload wasn't refreshed | Reload the page once to pick up the latest container ID; both surfaces read the same setting |
| Save rejected with a validation error | The container ID doesn't match `GTM-` followed by letters and digits | Copy the exact ID from the GTM workspace overview |
| Save blocked with a plan message | Your organization is on Free or still in the Grow trial | Upgrade the plan, then save |

## Related

- [Integrations overview](/settings/integrations) - the full integrations catalog.
- [Website](/content/website) - the CMS website surface where the container loads.
