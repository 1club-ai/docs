...
Edit anything (or change the theme) and a floating **Save** bar appears at the bottom of the page. Click it to commit. The theme is also saved immediately to your browser, so closing the page without saving still keeps your last theme - but it won't sync to other browsers until you save.

## Changing your password

Click **Change password** in the top right. The dialog asks for your current password and a new one. If you signed in with an OAuth provider (Google, Apple, Facebook) and never set a local password, contact an org admin to use the **Set password** action from **Settings > Users** instead.
